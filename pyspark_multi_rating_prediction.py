...
spark = SparkSession.builder \
    .appName("AmazonBooksMulticlassClassification") \
    .master("local[*]") \
...
# Convert 'r_score' column to numeric
ratings_df = ratings_df.withColumn("r_score", col("r_score").cast("int"))

# Drop rows with missing or invalid 'r_score' values
ratings_df = ratings_df.na.drop(subset=["r_score"])

# Drop rows with missing values in required columns
ratings_df = ratings_df.na.drop(subset=["r_review", "price", "r_score"])

# Convert 'price' to vector format for MinMaxScaler
...
# Define models
models = {
    "Logistic Regression": LogisticRegression(featuresCol="features", labelCol="r_score", maxIter=10),
    "Random Forest": RandomForestClassifier(featuresCol="features", labelCol="r_score", numTrees=10),
    "Decision Tree": DecisionTreeClassifier(featuresCol="features", labelCol="r_score")
}
...
def train_and_evaluate(model_name, model):
    ...
    evaluator_accuracy = MulticlassClassificationEvaluator(labelCol="r_score", predictionCol="prediction", metricName="accuracy")
    evaluator_precision = MulticlassClassificationEvaluator(labelCol="r_score", predictionCol="prediction", metricName="weightedPrecision")
    evaluator_recall = MulticlassClassificationEvaluator(labelCol="r_score", predictionCol="prediction", metricName="weightedRecall")
    evaluator_f1 = MulticlassClassificationEvaluator(labelCol="r_score", predictionCol="prediction", metricName="f1")
    ...
    return model_name, accuracy, precision, recall, f1, duration
...
with ThreadPoolExecutor(max_workers=4) as executor:
    ...
    for future in futures:
        name, accuracy, precision, recall, f1, duration = future.result()
...
parallel_time = max(duration for _, _, _, _, _, duration in results)
